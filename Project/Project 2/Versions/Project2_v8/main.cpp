...
char opening();
int dieRoll();
void play(Player &,Player &,int &,Property &,string &,int &,int &,short &,
        short &);
void Menu();
void prcsOpt(int,Player &,Property &,int &,int &);
void automat(Player &,Player &,int &,Property &,int &,int &,short &,short &);
...
int main(int argc, char** argv) {
    ...
    first=opening();
    if(first=='c') 
        automat(comp,user,compcur,prop,compMon,userMon,chance,comchst);
    cout<<"----------------------------------------"<<endl;
    
    //Begin Playing
    do{
        play(user,comp,current,prop,land,userMon,compMon,chance,comchst);
        automat(comp,user,compcur,prop,compMon,userMon,chance,comchst);
        cout<<"----------------------------------------"<<endl;
...
void automat(Player &c,Player &u,int &last,Property &prop,int &mon,int &umon,
        short &comindx,short &chnindx){
    ChncCom temp;
    ...
    if(prop.getname()=="Chance"){
        temp.setMess(1,chnindx,c,u,mon,umon,last);
        cout<<endl<<temp.getMess()<<endl;
        chnindx++;
    }
    else if(prop.getname()=="Community Chest"){
        temp.setMess(2,comindx,c,u,mon,umon,last);
        mon=c.getMony();
        if(u.getMony()!=1500)
            umon=u.getMony();
        cout<<endl<<temp.getMess()<<endl;
...
void prcsOpt(int decide,Player &u,Property &prop,int &mon,int &last){
...
void play(Player &u,Player &c,int &last,Property &spot,string &land,int &money,
        int &compmon,short &comindx,short &chnindx){  
    ...
    if(land=="Chance"){
        temp.setMess(1,chnindx,u,c,money,compmon,last);
        cout<<endl<<temp.getMess()<<endl;
        chnindx++;
    }
    else if(land=="Community Chest"){
        temp.setMess(2,comindx,u,c,money,compmon,last);
        money=u.getMony();
        if(c.getMony()!=1500)
            compmon=c.getMony();
        cout<<endl<<temp.getMess()<<endl;
        ...
        prcsOpt(choice,u,spot,money,last);
    }
}
