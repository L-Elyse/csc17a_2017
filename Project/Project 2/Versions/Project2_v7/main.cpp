...
char opening();
int dieRoll();
void play(Player,Player,int &,Property ,string &,int &,int &,short &,short &);
void Menu();
...
void automat(Player c,Player u,int &last,Property prop,int &mon,int &umon,
    ...
    if(prop.getname()=="Chance"){
        temp.setMess(1,chnindx,c,u,mon,umon);
        cout<<endl<<temp.getMess()<<endl;
        chnindx++;
    }
    else if(prop.getname()=="Community Chest"){
        temp.setMess(2,comindx,c,u,mon,umon);
        mon=c.getMony();
        if(u.getMony()!=1500)
            umon=u.getMony();
        cout<<endl<<temp.getMess()<<endl;
...
void play(Player u,Player c,int &last,Property spot,string &land,int &money,
    ...
    if(land=="Chance"){
        temp.setMess(1,chnindx,u,c,money,compmon);
        cout<<endl<<temp.getMess()<<endl;
        chnindx++;
    }
    else if(land=="Community Chest"){
        temp.setMess(2,comindx,u,c,money,compmon);
        money=u.getMony();
        if(c.getMony()!=1500)
            compmon=c.getMony();
        cout<<endl<<temp.getMess()<<endl;
